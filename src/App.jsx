...
export default function App(){
  ...
  async function doAddTrade(){
    ...
    setLoading(true);
    const {data,error}=await supabase.from('trades').insert({raro:tF.raro.trim(),quantidade:qtd,categoria:tF.categoria,preco_venda:pv,preco_por_unidade:Math.round(pv/qtd),data:tF.data,vendedor:tF.vendedor.trim(),comprador:tF.comprador.trim(),lancado_por:user.username,status:user.is_admin?'approved':'pending'}).select().single();
    setLoading(false);
